...
class MathsQuiz:
  ...
  def __init__(self,parent):
    ...
    Report_page = ["Name", "Age", "Score", "Date"] 
    self.report_labels = []
    ...
    self.user_age = Label(self.Report_frame, textvariable = self.name)
...
#Error checking for empty and age limit cases
          ...
          '''
          user_name = self.name
          self.WarningLabel.configure(text = user_name)
          '''
  ...
  def check_answer(self):
    ...
    if self.score <= 5:
      ...
      self.user_name.configure(text = str(self.name))
      self.user_age.configure(text = str(self.age))
